...
function abrirPopupAgrupada(marcadorClicado) {{
  ...
  // Só inclui marcadores que passaram nos filtros ativos (estão em `visiveis`)
  const idsVisiveis = new Set(visiveis.map(d => d.id));

  const lista = marcadores.filter(m => {{
    if (!m._d) return false;
    if (!idsVisiveis.has(m._d.id)) return false; // respeita filtros
  ...
  // Botão voltar mobile — empurra estado e escuta popstate
  history.pushState({{ popup: true }}, '');
}}

// Listener global de popstate — fecha popup ao apertar voltar
